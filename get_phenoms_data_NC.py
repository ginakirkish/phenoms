...
base_dir = '/data/henry12/phenoms/Newcastle/'
...
def write_csv(c,out):
    ...
    for idx in range(len(df)):
        ...
        first_path =  glob('/data/henry12/phenoms/Newcastle/first_output/{}*/'.format(ID))
        print(first_path)

        if len(first_path)>0:
            first_path = first_path[0]
            t1 = first_path.split(ID)[-1]
            df.loc[idx, "T1"] = t1
            print(t1, "T1")
        ...
        for sienax in os.listdir(base_dir + '/sienax_output/'):
            if sienax.startswith(ID):
                report = base_dir + '/sienax_output/'+ sienax + '/report.sienax'

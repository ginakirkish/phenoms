...
base_dir = '/data/henry12/phenoms/Charite/'
...
subjects = glob('{}/VIMS_001*/*VIMS_MS*'.format(base_dir))
for vim in subjects:
    #print(vim)
    for v in os.listdir(vim):
        sub_path = vim +'/'+ v
        for t1_dir in os.listdir(sub_path):
            if t1_dir.startswith("MPRAGE"):
                t1 = glob('{}/{}/o*.nii.gz'.format(sub_path, t1_dir))[0]
                print(t1)
                sub = t1.split('/')[6]
                print(sub)
                run_all(t1, sub)

                """dcm_folder = sub_path + '/'+ t1_dir + '/*'
                #print(dcm_folder)
                cmd = ["dcm2nii", dcm_folder]
                print("dcm2nii", dcm_folder)
                #Popen(cmd).wait()"""
